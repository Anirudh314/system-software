...
int num,len,actual_len,add,i,flag=0,p,l,t=0,j,cr=0,ct=0,rmdr,len1,adc,adr1;
...
//header record
fscanf(fp1,"%s%s%x",line,line,&adr1);
rewind(fp1);
fscanf(fp1,"%s",addr);
//printf("%s\n",addr);
printf("=-------------=-=-=-=-=-=-adr1 = %x",adr1);
...
fscanf(fp1,"%[^\n]\n",line);
	...
	if(j<69)
	{
		fprintf(fp5,"%6x^%x^",adr1,j+3);
		adr1=adr1+j+3;
	}
	else
	{
		fprintf(fp5,"%6x^%x^",adr1,69);
		adr1=adr1+69;
